...
. Создайте PKCS12-файл. Шаг можно пропустить, если уже есть сертификат в формате `.p12` или `.pfx`.
+
[source,bash]
----
openssl pkcs12 -export -in fullchain.pem -inkey privkey.pem -out keystore.p12 -passout pass:MyP@ssw0rd <.>
----
<.> Параметры:
+
* `-in fullchain.pem` - промежуточный и конечный сертификат
* `-inkey privkey.pem` - приватный ключ
* `-out keystore.p12` - имя выходного PKCS12-файла
* `-passout pass:MyP@ssw0rd` - пароль для защиты PKCS12-файла
+
...
EOF
----
<.> `server.ssl.key-store=/keystore.p12` -- путь до PKCS12-файла в контейнере
<.> `server.ssl.key-store-password=MyP@ssw0rd` -- пароль от PKCS12-файла
+
...
----
<.> `-v $(realpath application.properties):/application.properties` -- монтирование файла конфигурации `хост:контейнер`
<.> `-v $(realpath keystore.p12):/keystore.p12` -- монтирование PKCS12-файла `хост:контейнер`
<.> `--spring.config.additional-location=/application.properties` -- путь до файла конфигурации в контейнере
