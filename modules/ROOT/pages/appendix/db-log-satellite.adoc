...
[TIP]
====
* Основной способ подключения сателлитной БД -- создать её через _{mc}_, см. раздел "xref:dev@mgmtconsole:servers:db-satellite.adoc[]" с документации {of-mc}.
* В предыдущих версиях {dv} поддерживается возможность настроить архивирование через {cns}, см. раздел "xref:platform:console:db-satellite.adoc[]" с документации модуля {pl}.
====
