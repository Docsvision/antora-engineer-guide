...
. Предоставьте права на чтение файла `krb5.keytab` пользователям, от имени которых запускаются службы *{sss-new}* и *{wcs-new}*. Обратите внимание, что {wc} запускается от имени учетной записи {dv} в версии {cu253-wc-req} и выше.

[#client]
== Клиентские настройки (опционально)

Следующие настройки являются опциональными и необходимы, когда требуется выполнять аутентификацию от имени пользователя вне домена (может быть полезно в диагностических целях).
