...
WARNING: Особенности протокола аутентификации Kerberos требуют указывать адреса сервисов {dv} в конфигурационных файлах с доменным именем. Обращение по IP-адресу сервиса звершится ошибкой.
