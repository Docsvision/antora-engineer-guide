...
{dv} {wc} поддерживает авторизацию с помощью сторонних расширений. Необходимые ресурсы для работы расширений авторизации в {wc}е поставляются с модулем {pl} и могут настраиваться двумя способами.

****
Начиная с модуля {pl} версии {pl-cu-mc}, настройку расширений авторизации можно выполнить двумя способами:

* При помощи редактирования конфигурационного файла модуля {pl}, см. подробнее ниже
* Из интерфейса {of-mc}, см. подробнее раздел "xref:dev@mgmtconsole:servers:appserver-auth-extensions.adoc[]" в документации {of-mc}
****
...
+
.. Установить сертификат площадки из архива http://esia.gosuslugi.ru/public/esia.zip[esia.zip] в хранилище пользователей, под которыми запускаются сервисы *{sss-new}* и *{wcs-new}*. Сертификат тестовой площадки -- `TESIA GOST 2012 new.cer`, например, так:
+
[source,bash]
----
