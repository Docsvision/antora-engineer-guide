...
// tag::idb[]
[#idb]
== Регистрация расширения IdentityBlitz

NOTE: Для работы расширения авторизации IdentityBlitz требуется опция лицензии _{dv} Коннектор к Blitz Identity Provider_.

{dv} версии {idb-req} поддерживает возможность использовать группы из внешнего провайдера IdentityBlitz. При настроенном коннекторе к IdentityBlitz права доступа в {dv} автоматически назначаются на основе членства пользователя во внешних группах, аналогично Active Directory, настройки в справочнике сотрудников не потребуются.

[#idb-settings]
=== Строка настроек IdentityBlitz
