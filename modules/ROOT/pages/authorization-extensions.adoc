...
+
.. Добавить настройки для аутентификации через ЕСИА в конфигурационный файл модуля {pl}.
+
...
* Авторизация с помощью Keycloak поддерживается в {wc}е {kc-wc-req} при установке модуля {pl} {kc-pl-req}.
====
        ...
        "Authentication": {
          "Extensions": {
		    "Keycloak": {
              ...
              "Settings": "<?xml version=\"1.0\" encoding=\"utf-16\"?><KeycloakAuthenticationSettings  xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"><WellKnownConfigurationUrl>https://vdv-keycloak.digdes.com/realms/testDv/.well-known/openid-configuration</WellKnownConfigurationUrl><ClientId>test</ClientId><Secret>JwIcDLOLEATB2R69bZac9BMHpzC8Px0P</Secret><AccountNameClaim>http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress</AccountNameClaim><UniqueIdClaim>DocsvisionID</UniqueIdClaim></KeycloakAuthenticationSettings>", <.>
...
[#ldaps]
== Регистрация расширения LDAPS

[NOTE]
====
* Для работы расширения авторизации LDAPS требуется опция лицензии _{dv} Коннектор к LDAPS_.
* Автоизация с помощью LDAPS поддерживается в {wc}е {kc-wc-req} при установке модуля {pl} {kc-pl-req}.
====

.Особенности и рекомендации
****
* Расширение LDAPS имеет особенности именования. Расширение плохо поддерживает пробелы и заглавные буквы в именах пользователей и групп, а также прочие стандартные ограничения.
+
...
+
* Аналогичные ограничения касаются запросов к контроллеру домена -- все составные части следует писать строчными буквами, без лишних пробелов и лишних кавычек. Исключение -- имена пользователей, групп и прочих объектов, которые точно совпадают по регистру, иначе имена будут считаться различными.
* Особенности доступа. Всегда обязательно нужно указывать верные `DistinguishedName`. Это часть `dc=astrald,dc=ru`. В нашем случае она так выглядит, т.к. домен имеет вид `astrald.ru`. Если он будет, например `astrald.example.domain.ru`, то должно быть так: `dc=astrald,dc=example,dc=domain,dc=ru`. +
...
----
{
  "Authentication": {
	...
	"Tenants": { <.>
		"docsvision-db": {
		  "Extensions": "{36351525-8FCE-48AB-8AD3-4C1FA575FF77; 36351525-8FCE-48AB-8AD3-4C1FA575FF77}", <.>
		  "Name": "CurrentDB" <.>
		}
	}
  }
