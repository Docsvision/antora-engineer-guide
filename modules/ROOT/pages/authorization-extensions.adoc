...
<.> Переключиться на пользователя _docsvision_ для службы *{wcs-new}*.
+
.. Установите сертификат площадки из архива http://esia.gosuslugi.ru/public/esia.zip[esia.zip] в хранилище пользователей, под которыми запускаются сервисы *{sss-new}* и *{wcs-new}*. Сертификат тестовой площадки -- `TESIA GOST 2012 new.cer`, например, так:
+
[source,bash]
----
